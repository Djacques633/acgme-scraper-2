...
def get_pages(stateId):
    ...
    urls = ['{}{}'.format("https://apps.acgme.org", row['href'])
            for row in row_urls]
    return urls
...
def scrape_program(url, sheet1, y, stateId):
    ...
    code = int(
        soup.find('dl', attrs={'class': "dl-horizontal"}
                  ).find('dd').find_next('a')
            .contents[0].strip())
    phone = ''
    email = ''
    try:
        phone = list(soup.findAll(
            'dl', attrs={'class': "dl-horizontal"}))[2].find('dd').contents[0].strip()
    except:
        phone = 'Could not find!'
    try:
        email = list(soup.findAll('dl', attrs={
                     'class': "dl-horizontal"}))[2].find('dd').find_next('a').contents[0].strip()
    except:
        email = 'Could not find!'
    website = soup.findAll('a')[2].get('href')

    y = add_to_excel(sheet1, y, program_number, institution, specialty,
                     location, city, state, zip_code, address, website, code, phone, email)
    return y


def getUserInputStates():

    print("Enter states to scrape from split by one comma:")
    states = input().split(',')
    return states


dict = {}
with open("stateId.json", "r") as config_file:
    dict = json.load(config_file)
states = getUserInputStates()
...
for x in range(0, len(states)):
    stateId = dict[states[x].lower()]
    print("Fetching pages from the ACGME Program search")
    pages = get_pages(stateId)
    print("Finished! Setting up excel sheet...")
    for p in range(0, len(pages)):
        if p % 10 == 0:
            print("Scraping page " + str(p) + " of " +
                  str(len(pages)) + " for ID: " + states[x])
        try:
            y = scrape_program(pages[p], sheet1, y, stateId)
        except:
            f.close()
            exit()
f.close()
